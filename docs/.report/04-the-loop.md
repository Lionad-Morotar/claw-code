...
真正驱动循环的是 `run_turn` 方法，起始于 [`conversation.rs#L296`](/rust/crates/runtime/src/conversation.rs#L296)。`run_turn` 实际定义在 [`conversation.rs#L296-L484`](/rust/crates/runtime/src/conversation.rs#L296-L484)。它的结构可以概括为：
...
以下是 `run_turn` 的相位拆解，对应 [`conversation.rs#L296-L484`](/rust/crates/runtime/src/conversation.rs#L296-L484) 附近的源码逻辑：
...
其中 `ContentBlock`（[`session.rs#L28-L52`](/rust/crates/runtime/src/session.rs#L28-L52)）是结构化内容的枚举：
...
自动压缩逻辑在 [`conversation.rs#L522-L542`](/rust/crates/runtime/src/conversation.rs#L522-L542)：
...
`compact_session` 定义于 [`runtime/src/compact.rs#L96-L155`](/rust/crates/runtime/src/compact.rs#L96-L155)。其策略是：
...
`ApiClient` 定义于 [`conversation.rs#L56-L61`](/rust/crates/runtime/src/conversation.rs#L56-L61)：
...
`build_assistant_message` 负责将一维事件序列还原为结构化的 `ConversationMessage`，定义于 [`conversation.rs#L672-L771`](/rust/crates/runtime/src/conversation.rs#L672-L771)：
...
在 `run_turn` 中，每个 pending tool use 都会经过以下代码块（[`conversation.rs#L348-L470`](/rust/crates/runtime/src/conversation.rs#L348-L470)）：
...
每次 `build_assistant_message` 解析出 `Usage` 事件时，`run_turn` 会调用 `self.usage_tracker.record(usage)`（[`conversation.rs#L335-L338`](/rust/crates/runtime/src/conversation.rs#L335-L338)）。`maybe_auto_compact` 以 `cumulative_usage().input_tokens` 作为触发条件，这本质上就是一种基于累计 token 的预算管理。
