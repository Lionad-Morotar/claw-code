...
`ConversationRuntime` 定义于 [`runtime/src/conversation.rs#L126-L150`](/rust/crates/runtime/src/conversation.rs#L126-L150)：
...
真正驱动循环的是 `run_turn` 方法，起始于 [`conversation.rs#L296`](/rust/crates/runtime/src/conversation.rs#L296)。它的结构可以概括为：
...
消息存储在 `Session::messages` 中，类型为 `Vec<ConversationMessage>`。`ConversationMessage` 定义于 [`runtime/src/session.rs#L47-L52`](/rust/crates/runtime/src/session.rs#L47-L52)：
...
其中 `ContentBlock`（[`session.rs#L28-L43`](/rust/crates/runtime/src/session.rs#L28-L43)）是结构化内容的枚举：
...
自动压缩逻辑在 [`conversation.rs#L548-L568`](/rust/crates/runtime/src/conversation.rs#L548-L568)：
...
默认阈值由常量 [`DEFAULT_AUTO_COMPACTION_INPUT_TOKENS_THRESHOLD`](/rust/crates/runtime/src/conversation.rs#L15) 设定为 `100_000`，也可以通过环境变量 `CLAUDE_CODE_AUTO_COMPACT_INPUT_TOKENS` 覆盖。这与文档中描述的“autocompact 超出阈值时触发”完全一致，只是触发时机放在了一轮 turn 结束之后，而非每次流式调用之前。

### 源码映射：`compact_session` 的实现

`compact_session` 定义于 [`runtime/src/compact.rs#L96-L133`](/rust/crates/runtime/src/compact.rs#L96-L133)。其策略是：
...
`ApiClient` 定义于 [`conversation.rs#L62-L64`](/rust/crates/runtime/src/conversation.rs#L62-L64)：
...
`AssistantEvent` 枚举（[`conversation.rs#L33-L48`](/rust/crates/runtime/src/conversation.rs#L33-L48)）涵盖了一次 assistant turn 中可能收到的全部事件：
...
`build_assistant_message` 负责将一维事件序列还原为结构化的 `ConversationMessage`，定义于 [`conversation.rs#L733-L777`](/rust/crates/runtime/src/conversation.rs#L733-L777)：
...
`PromptCacheEvent`（[`conversation.rs#L50-L58`](/rust/crates/runtime/src/conversation.rs#L50-L58)）记录了缓存异常的诊断信息：
...
`run_turn` 会将每个 turn 产生的 `prompt_cache_events` 累积到 `TurnSummary` 中，供上层做缓存稳定性诊断。测试代码（[`conversation.rs#L878-L910`](/rust/crates/runtime/src/conversation.rs#L878-L910)）演示了一个 `unexpected: true` 的场景，验证了缓存 token 下降 5,000 时的事件捕获能力。
...
在 `run_turn` 中，每个 pending tool use 都会经过以下代码块（[`conversation.rs#L348-L430`](/rust/crates/runtime/src/conversation.rs#L348-L430)）：
...
权限核心逻辑在 [`runtime/src/permissions.rs#L175-L250`](/rust/crates/runtime/src/permissions.rs#L175-L250) 的 `authorize_with_context` 中。它的判定顺序是：
...
`HookRunner` 定义于 [`runtime/src/hooks.rs#L90-L987`](/rust/crates/runtime/src/hooks.rs)。支持三类钩子：
...
在 [`conversation.rs#L340-L346`](/rust/crates/runtime/src/conversation.rs#L340-L346)：
...
定义见 [`conversation.rs#L107-L117`](/rust/crates/runtime/src/conversation.rs#L107-L117)。
...
| 终止原因 | 源码位置 | 机制 |
|----------|----------|------|
| **max_iterations 超出** | [`conversation.rs#L308-L314`](/rust/crates/runtime/src/conversation.rs#L308-L314) | `iterations > self.max_iterations` → 返回 `RuntimeError` |
...
`max_iterations` 默认值为 `usize::MAX`（[`conversation.rs#L155`](/rust/crates/runtime/src/conversation.rs#L155)），但可以通过 builder 方法限制：
...
测试代码 [`conversation.rs#L1579-L1608`](/rust/crates/runtime/src/conversation.rs#L1579-L1608) 验证了一个循环 API 客户端在 `max_iterations = 1` 时的行为——当第一个 turn 永远返回 `ToolUse` 时，第二个迭代就会触发超限错误。
...
每次 `build_assistant_message` 解析出 `Usage` 事件时，`run_turn` 会调用 `self.usage_tracker.record(usage)`（[`conversation.rs#L334`](/rust/crates/runtime/src/conversation.rs#L334)）。`maybe_auto_compact` 以 `cumulative_usage().input_tokens` 作为触发条件，这本质上就是一种基于累计 token 的预算管理。
