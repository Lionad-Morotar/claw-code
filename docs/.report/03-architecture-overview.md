...
[`rusty-claude-cli/src/main.rs#L180-L395`](/rust/crates/rusty-claude-cli/src/main.rs#L180-L395) 的 `run()` 函数是进程级入口。它把命令行参数解析为 `CliAction` 枚举，随后按模式分发：
...
[`run_repl`](/rust/crates/rusty-claude-cli/src/main.rs#L3143-L3219) 创建一个 [`input::LineEditor`](/rust/crates/rusty-claude-cli/src/input.rs) 循环读取终端输入，遇到斜杠命令时由 [`SlashCommand::parse`](/rust/crates/commands/src/lib.rs) 处理，普通文本则进入 `LiveCli::run_turn()`。

#### 管道输入的合并策略

在 [`CliAction::Prompt`](/rust/crates/rusty-claude-cli/src/main.rs#L219-L247) 分支中，系统通过 [`read_piped_stdin`](/rust/crates/rusty-claude-cli/src/main.rs#L146-L165) 检测 stdin 是否为终端。只有当 `permission_mode` 为 `DangerFullAccess`（完全无人值守模式）时，才会读取管道输入并与 prompt 合并——否则 stdin 需要保留给交互式权限确认使用。
...
在调用 `run_turn` 之前，[`prepare_turn_runtime`](/rust/crates/rusty-claude-cli/src/main.rs#L3819-L3840) 完成运行时的"冷启动"组装：
...
`claw-code` 的工具层实现位于 `tools` crate。[`execute_tool`](/rust/crates/tools/src/lib.rs#L1174-L1265) 是一个巨大的 `match name { ... }`，直接映射 30+ 个显式内置工具 + MCP 动态扩展：
...
在 [`consume_stream`](/rust/crates/rusty-claude-cli/src/main.rs#L7059-L7223) 中，文本增量到达后立即调用 `write!(out, "{rendered}")` 并 `flush`，实现真正的低延迟打字机效果。
...
| **参数解析** | [`rusty-claude-cli/src/main.rs#L396-L714`](/rust/crates/rusty-claude-cli/src/main.rs#L396-L714) | `parse_args()`，构建 `CliAction` |
| **REPL 循环** | [`rusty-claude-cli/src/main.rs#L3143-L3219`](/rust/crates/rusty-claude-cli/src/main.rs#L3143-L3219) | `run_repl()`，`LineEditor` 读取终端输入 |
| **单次 Prompt** | [`rusty-claude-cli/src/main.rs#L219-L247`](/rust/crates/rusty-claude-cli/src/main.rs#L219-L247) | `CliAction::Prompt` 分支，管道输入合并 |
| **运行时组装** | [`rusty-claude-cli/src/main.rs#L6726-L6753`](/rust/crates/rusty-claude-cli/src/main.rs#L6726-L6753) | `build_runtime()` → `ConversationRuntime` 组装 |
...
当 `permission_mode` 为 `DangerFullAccess` 时，[`read_piped_stdin`](/rust/crates/rusty-claude-cli/src/main.rs#L146-L165) 会将 stdin 内容附加到 prompt 之后。这让 `claw-code` 能无缝嵌入 CI/CD 脚本和自动化流程。
