...
在调用 `run_turn` 之前，[`prepare_turn_runtime`](/rust/crates/rusty-claude-cli/src/main.rs#L3442-L3460) 完成运行时的"冷启动"组装：
...
[`build_assistant_message`](/rust/crates/runtime/src/conversation.rs#L672-L771) 将离散的 `AssistantEvent` 聚合成结构化的 `ConversationMessage`：
...
在每轮 turn 结束时，[`maybe_auto_compact`](/rust/crates/runtime/src/conversation.rs#L522-L542) 会检查 `usage_tracker.cumulative_usage().input_tokens` 是否超过阈值（默认 100,000）。若超过，则调用 [`compact_session`](/rust/crates/runtime/src/compact.rs#L96-L155) 将历史消息压缩为一条系统摘要消息，控制上下文长度和 API 成本。
...
[`ProviderClient::from_model_with_anthropic_auth`](/rust/crates/api/src/client.rs#L21-L48) 根据 `detect_provider_kind()` 决定路由：
...
所有后端都实现 [`Provider`](/rust/crates/api/src/providers/mod.rs#L14-L30) trait：
...
| **运行时组装** | [`rusty-claude-cli/src/main.rs#L6225-L6305`](/rust/crates/rusty-claude-cli/src/main.rs#L6225-L6305) | `build_runtime()` → `ConversationRuntime` 组装 |
