...
| **核心循环层** | 单轮：发请求 → 拿响应 → 执行工具 → 循环 | [`runtime/src/conversation.rs#L296-L480`](/rust/crates/runtime/src/conversation.rs#L296-L480) | Agentic Loop、turn |
...
[`rusty-claude-cli/src/main.rs#L165-L235`](/rust/crates/rusty-claude-cli/src/main.rs#L165-L235) 的 `run()` 函数是进程级入口。它把命令行参数解析为 `CliAction` 枚举，随后按模式分发：
...
[`run_repl`](/rust/crates/rusty-claude-cli/src/main.rs#L2770-L2825) 创建一个 [`input::LineEditor`](/rust/crates/rusty-claude-cli/src/input.rs) 循环读取终端输入，遇到斜杠命令时由 [`SlashCommand::parse`](/rust/crates/commands/src/lib.rs) 处理，普通文本则进入 `LiveCli::run_turn()`。
...
进入 `LiveCli` 后，真正的编排核心在 [`runtime/src/conversation.rs#L126-L150`](/rust/crates/runtime/src/conversation.rs#L126-L150) 的 `ConversationRuntime<C, T>` 中。

#### LiveCli 到 Runtime 的组装链

在调用 `run_turn` 之前，[`prepare_turn_runtime`](/rust/crates/rusty-claude-cli/src/main.rs#L3416-L3430) 完成运行时的"冷启动"组装：
...
[`ConversationRuntime`](/rust/crates/runtime/src/conversation.rs#L126-L150) 持有 11 个字段：
...
[`run_turn`](/rust/crates/runtime/src/conversation.rs#L296-L480) 是核心循环的核心。每个 turn 内部是一个 `loop { ... }`，典型流程如下：
...
[`build_assistant_message`](/rust/crates/runtime/src/conversation.rs#L672-L710) 将离散的 `AssistantEvent` 聚合成结构化的 `ConversationMessage`：
...
在 [`run_turn`](/rust/crates/runtime/src/conversation.rs#L360-L410) 的工具执行循环中，每个 ToolUse 需要经过两个关卡：
...
在每轮 turn 结束时，[`maybe_auto_compact`](/rust/crates/runtime/src/conversation.rs#L521-L545) 会检查 `usage_tracker.cumulative_usage().input_tokens` 是否超过阈值（默认 100,000）。若超过，则调用 [`compact_session`](/rust/crates/runtime/src/compact.rs#L117-L155) 将历史消息压缩为一条系统摘要消息，控制上下文长度和 API 成本。

---

### 4. 工具层（tools/src/lib.rs）

`claw-code` 的工具层实现位于 `tools` crate。[`execute_tool`](/rust/crates/tools/src/lib.rs#L1174-L1260) 是一个巨大的 `match name { ... }`，直接映射 40+ 个内置工具：
...
在 CLI 路径中，工具执行器是 `CliToolExecutor`（在 `main.rs` 内部定义，通过 `build_runtime_with_plugin_state` 注入）。在子 Agent 路径中，工具执行器是 [`SubagentToolExecutor`](/rust/crates/tools/src/lib.rs#L3951-L3970) —— 它会先检查 `allowed_tools` 白名单，再调用 `execute_tool_with_enforcer`。
...
[`runtime/src/bash.rs#L19-L67`](/rust/crates/runtime/src/bash.rs#L19-L67) 定义了 `BashCommandInput` schema：
...
沙箱状态由 [`sandbox_status_for_input`](/rust/crates/runtime/src/bash.rs#L140-L180) 根据运行时配置和模型请求共同决定。默认情况下命令在受限环境中执行；模型可以显式请求关闭沙箱（需要更高权限）。异步执行使用 `tokio::process::Command` + `tokio::time::timeout`，输出超过阈值时会被截断。
...
`api` crate 的职责是与上游模型接口通信。[`ProviderClient`](/rust/crates/api/src/client.rs#L13-L21) 是一个 enum，覆盖三种后端：
...
[`ProviderClient::from_model_with_anthropic_auth`](/rust/crates/api/src/client.rs#L22-L55) 根据 `detect_provider_kind()` 决定路由：
...
所有后端都实现 [`Provider`](/rust/crates/api/src/providers/mod.rs#L14-L25) trait：
...
在 CLI 侧，[`AnthropicRuntimeClient::consume_stream`](/rust/crates/rusty-claude-cli/src/main.rs#L6530-L6620) 把 `stream.next_event()` 的 SSE 事件实时转换为终端输出：

- `ContentBlockDelta::TextDelta` → 通过 [`MarkdownStreamState::push`](/rust/crates/rusty-claude-cli/src/render.rs#L607-L620) 增量渲染为 ANSI Markdown
...
在 [`consume_stream`](/rust/crates/rusty-claude-cli/src/main.rs#L6530-L6620) 中，文本增量到达后立即调用 `write!(out, "{rendered}")` 并 `flush`，实现真正的低延迟打字机效果。
...
[`PermissionMode`](/rust/crates/runtime/src/permissions.rs#L9-L16) 定义了五级模式：
...
会话记忆由 `Session` 持久化到 `~/.local/share/opencode/*.jsonl`，支持 `--resume` 恢复和 `fork` 分支。
...
| 入口 | 文件 | 说明 |
| --- | --- | --- |
| **CLI 启动** | [`rusty-claude-cli/src/main.rs#L1-L50`](/rust/crates/rusty-claude-cli/src/main.rs#L1-L50) | `fn main()`，全局导入，错误统一退出处理 |
| **参数解析** | [`rusty-claude-cli/src/main.rs#L371-L520`](/rust/crates/rusty-claude-cli/src/main.rs#L371-L520) | `parse_args()`，构建 `CliAction` |
| **REPL 循环** | [`rusty-claude-cli/src/main.rs#L2770-L2825`](/rust/crates/rusty-claude-cli/src/main.rs#L2770-L2825) | `run_repl()`，`LineEditor` 读取终端输入 |
...
| **运行时组装** | [`rusty-claude-cli/src/main.rs#L6209-L6280`](/rust/crates/rusty-claude-cli/src/main.rs#L6209-L6280) | `build_runtime()` → `ConversationRuntime` 组装 |
