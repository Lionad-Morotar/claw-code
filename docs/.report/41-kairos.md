...
- `main.tsx:1793-1853` — `--assistant` 标志处理与 `initializeAssistantTeam()` 调用
- `main.tsx:2550-2660` — `--channels` / `--dangerously-load-development-channels` 解析
- `main.tsx:2662-2680` — `--tools` 中包含 `SendUserMessage` 时的 opt-in 处理
- `main.tsx:3306-3320` — `defaultView === 'chat'` 时的 Brief opt-in
- `main.tsx:3323-3343` — `--proactive` 时追加 proactive prompt
- `main.tsx:3345-3351` — `getAssistantSystemPromptAddendum()` 注入
- `main.tsx:3741-3744` — `assistantActivationPath` 传给 `createQueryEngine()`
...
- `src/bridge/replBridge.ts#L1378-1503` — v1 `HybridTransport`（WebSocket 读 + POST 写）与 v2 `CCRClient`（SSETransport）的建立逻辑
...
| 概念 | 文件 | 行号范围 |
|------|------|---------|
| `feature('KAIROS')` 入口分支 | `src/main.tsx` | `142-145`, `1004-1019`, `1793-1853`, `2550-2660`, `3306-3351`, `3741-3744` |
...
| Transport 建立 | `src/bridge/replBridge.ts` | `1378-1503` |
