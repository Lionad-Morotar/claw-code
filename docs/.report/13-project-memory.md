...
在 `claw-code` 中，项目级上下文被封装为 `runtime/src/prompt.rs` 中的 `ProjectContext` 结构体（[`L55-L63`](/rust/crates/runtime/src/prompt.rs#L55-L62)）：
...
1. **自底向上遍历再反转**：从当前目录向上收集到根目录，再反转顺序处理。这样父级配置先进入 prompt，子级配置随后加载并可以覆盖。
2. **去重策略**：`dedupe_instruction_files`（[`L353-L368`](/rust/crates/runtime/src/prompt.rs#L353-L368)）使用 `stable_content_hash` 去重，避免同一内容在多个层级重复出现而浪费 token。
...
`ProjectContext::discover_with_git`（[`L81-L90`](/rust/crates/runtime/src/prompt.rs#L81-L90)）在基础发现之上追加三层 git 快照：
...
- `read_git_status`（[`L238-L255`](/rust/crates/runtime/src/prompt.rs#L238-L254)）：抓取 `git status --short --branch`
...
`Session` 结构体定义于 [`session.rs#L89-L100`](/rust/crates/runtime/src/session.rs#L89-L100)（节选）：
...
文件格式为 JSON Lines，支持四种 record type：
...
`session_control.rs` 中的 `SessionStore`（[`L19-L63`](/rust/crates/runtime/src/session_control.rs#L19-L46)）实现了按工作区指纹隔离的会话目录：
...
- **恢复**：`SessionStore::load_session` → `Session::load_from_path`（[`session.rs#L204-L219`](/rust/crates/runtime/src/session.rs#L204-L218)）支持从 JSON 或 JSONL 恢复。
...
当对话变长时，`ConversationRuntime` 会在 `run_turn` 结束后自动检查 token 阈值（默认 100k input tokens），触发 `maybe_auto_compact`（[`conversation.rs#L525-L543`](/rust/crates/runtime/src/conversation.rs#L525-L542)）：
