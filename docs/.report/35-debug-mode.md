...
### 为什么 Rust 实现没有 Bun `--inspect-wait`？
...
1. **会话中尚无工具调用**：返回 `"no prior tool call found in session"` 错误字符串。
2. **ToolUse 存在但 ToolResult 缺失**：报告会标记 `Result: missing tool result`，帮助排查工具执行是否被中断或挂起。
3. **结果为错误**：报告会显示错误类型和消息，便于快速定位问题。
...
- `JsonlTelemetrySink`：生产用，每条事件一行 JSON，append 到文件。参见 [`telemetry/src/lib.rs#L233-L278`](/rust/crates/telemetry/src/lib.rs#L233-L277)。
...
这种设计让"verbose 输出"不再是往 stderr 打印非结构化文本，而是产出**可机器解析的事件流**。其设计 rationale 在于：1) CLI 与 API 的交互链路复杂，纯文本日志难以在事后还原完整的 turn/tool 序列；2) 结构化 JSONL 可以直接导入分析 pipeline，用于定位高频失败工具、调试 token 消耗异常或回放特定会话；3) 当 `session_tracer` 为 `None` 时，整个追踪路径零开销，不会影响正常执行性能。
