...
请求的 `id` 可以是数字、字符串或 Null；`McpServerManager` 内部使用自增的 `u64` 作为 `JsonRpcId::Number`（[`mcp_stdio.rs#L963-L968`](/rust/crates/runtime/src/mcp_stdio.rs#L963-L968)）。
...
参见 [`mcp_stdio.rs#L521-L529`](/rust/crates/runtime/src/mcp_stdio.rs#L521-L529)。状态机非常简单：`process` 存在且 `initialized == true` 时，该服务器才被认为可用。
...
`McpServerManager::call_tool`（[`mcp_stdio.rs#L718-L773`](/rust/crates/runtime/src/mcp_stdio.rs#L718-L773)）的执行链路如下：
...
所有 MCP 相关错误都被封装为 `McpServerManagerError`（[`mcp_stdio.rs#L184-L232`](/rust/crates/runtime/src/mcp_stdio.rs#L184-L232)）：
...
`mcp_lifecycle_hardened.rs` 定义了 11 个生命周期阶段（[`mcp_lifecycle_hardened.rs#L14-L34`](/rust/crates/runtime/src/mcp_lifecycle_hardened.rs#L14-L34)）：
...
在 [`rusty-claude-cli/src/main.rs#L2971-L2984`](/rust/crates/rusty-claude-cli/src/main.rs#L2971-L2984) 中，CLI 启动时会调用 `RuntimeMcpState::new`：
...
MCP 工具的权限级别由 `tool.annotations` 决定（[`main.rs#L3204-L3223`](/rust/crates/rusty-claude-cli/src/main.rs#L3204-L3223)）：
...
即使某个 MCP 工具因服务器暂时不可用未被发现，CLI 仍注册了 3 个通用包装工具（[`main.rs#L3171-L3201`](/rust/crates/rusty-claude-cli/src/main.rs#L3171-L3201)）：
...
- `tools/list` —— 返回预注册的 `McpTool` 描述符列表（[`mcp_server.rs#L179-L190`](/rust/crates/runtime/src/mcp_server.rs#L179-L190)）
- `tools/call` —— 调用用户提供的 `ToolCallHandler`（[`mcp_server.rs#L192-L229`](/rust/crates/runtime/src/mcp_server.rs#L192-L229)）
...
if server_config.transport() == McpTransport::Stdio {
    ...
    unsupported_servers.push(...);
}
```

参见 [`mcp_stdio.rs#L531-L548`](/rust/crates/runtime/src/mcp_stdio.rs#L531-L548)。这意味着配置中的 `sse`、`http`、`ws`、`sdk`、`claudeai-proxy` 服务器目前**不会建立连接**，只会生成降级报告。
