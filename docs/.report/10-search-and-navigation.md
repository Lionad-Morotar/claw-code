...
`runtime/src/file_ops.rs` 的 `grep_search` 函数（[`L343-L431`](/rust/crates/runtime/src/file_ops.rs#L343-L431)）就是进程内正则搜索引擎：
...
`collect_search_files`（[`L452-L462`](/rust/crates/runtime/src/file_ops.rs#L452-L462)）使用 `walkdir` 递归遍历目录：
...
`grep_search` 对文件名和匹配行都会调用 `apply_limit`（[`L489-L507`](/rust/crates/runtime/src/file_ops.rs#L489-L507)）：
...
`WebSearch` 的实现位于 `tools/src/lib.rs` 的 `execute_web_search`（[`L2588-L2636`](/rust/crates/tools/src/lib.rs#L2588-L2636)）。与 TypeScript 上游的适配器模式不同，Rust 实现采用了一个轻量级的统一后端：
...
`WebFetch` 的实现位于 `execute_web_fetch`（[`L2556-L2587`](/rust/crates/tools/src/lib.rs#L2556-L2587)）:
...
- HTML 到纯文本的转换是一个自研的轻量级 parser（`html_to_text`，[`L2743-L2766`](/rust/crates/tools/src/lib.rs#L2743-L2766)），而非 Turndown

### 搜索结果解析

`extract_search_hits`（[`L2769-L2805`](/rust/crates/tools/src/lib.rs#L2769-L2805)）通过字符串扫描提取 DuckDuckGo 结果页中的 `result__a` 锚点。如果主模式找不到任何结果，会退化为 `extract_search_hits_from_generic_links`（[`L2807-L2838`](/rust/crates/tools/src/lib.rs#L2807-L2838)）——一个通用的 `<a href="...">` 抓取 fallback。
...
DuckDuckGo 的重定向解码在 `decode_duckduckgo_redirect`（[`L2883-L2901`](/rust/crates/tools/src/lib.rs#L2883-L2901)）中处理：支持直接的 `http/https` URL、`//` 协议相对 URL、`/l/` 短链（从 `uddg` 参数提取真实目标）。
