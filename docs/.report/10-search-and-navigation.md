...
- `GrepSearchInput`（[`L131-L154`](/rust/crates/runtime/src/file_ops.rs#L131-L154)）：
...
- `GlobSearchOutput`（[`L120-L128`](/rust/crates/runtime/src/file_ops.rs#L120-L128)）和 `GrepSearchOutput`（[`L157-L173`](/rust/crates/runtime/src/file_ops.rs#L157-L173)）返回搜索的元数据、匹配文件列表以及可选内容。
...
当模型请求 `glob_search` 或 `grep_search` 时，`tools/src/lib.rs` 的 `execute_tool_with_enforcer` 函数（[`L1178-L1207`](/rust/crates/tools/src/lib.rs#L1178-L1207) 附近）会按名分发：
...
`runtime/src/file_ops.rs` 的 `grep_search` 函数（[`L343-L450`](/rust/crates/runtime/src/file_ops.rs#L343-L450)）就是进程内正则搜索引擎：
...
`collect_search_files`（[`L452-L465`](/rust/crates/runtime/src/file_ops.rs#L452-L465)）使用 `walkdir` 递归遍历目录：
...
```
 ...
 glob 过滤由 `matches_optional_filters`（[`L467-L487`](/rust/crates/runtime/src/file_ops.rs#L467-L487)）完成，支持 `glob` 模式和 `type`（扩展名）两类过滤：
...
`grep_search` 对文件名和匹配行都会调用 `apply_limit`（[`L489-L508`](/rust/crates/runtime/src/file_ops.rs#L489-L508)）：
...
`glob_search` 函数在收集结果后，会对文件按 `mtime`（修改时间）降序排列（[`L320-L325`](/rust/crates/runtime/src/file_ops.rs#L320-L325)）：
...
1. **Schema 级错误**：`RegexBuilder::build()` 失败时返回 `io::ErrorKind::InvalidInput`（[`L351-L355`](/rust/crates/runtime/src/file_ops.rs#L351-L355)）。
2. **IO 级错误**：文件读取失败（如权限不足、二进制内容）时，`grep_search` 通过 `let Ok(file_contents) = fs::read_to_string(...)` 静默跳过该文件（[`L379-L381`](/rust/crates/runtime/src/file_ops.rs#L379-L381)），而不是让整个搜索崩溃。
...
`tools/src/lib.rs` 的核心搜索逻辑在 `search_tool_specs`（[`L240-L314`](/rust/crates/tools/src/lib.rs#L240-L314)）：
...
`GlobalToolRegistry::searchable_tool_specs`（[`L356-L371`](/rust/crates/tools/src/lib.rs#L356-L371)）只扫描**延迟工具**（deferred tools）：
...
`WebSearch` 的实现位于 `tools/src/lib.rs` 的 `execute_web_search`（[`L2590-L2635`](/rust/crates/tools/src/lib.rs#L2590-L2635)）。与 TypeScript 上游的适配器模式不同，Rust 实现采用了一个轻量级的统一后端：
...
`WebFetch` 的实现位于 `execute_web_fetch`（[`L2556-L2588`](/rust/crates/tools/src/lib.rs#L2556-L2588)）:
...
HTTP 客户端配置在 `build_http_client`（[`L2637-L2645`](/rust/crates/tools/src/lib.rs#L2637-L2645)）中：
...
- HTML 到纯文本的转换是一个自研的轻量级 parser（`html_to_text`，[`L2729-L2767`](/rust/crates/tools/src/lib.rs#L2729-L2767)），而非 Turndown

### 搜索结果解析

`extract_search_hits`（[`L2769-L2797`](/rust/crates/tools/src/lib.rs#L2769-L2797)）通过字符串扫描提取 DuckDuckGo 结果页中的 `result__a` 锚点。如果主模式找不到任何结果，会退化为 `extract_search_hits_from_generic_links`（[`L2799-L2833`](/rust/crates/tools/src/lib.rs#L2799-L2833)）——一个通用的 `<a href="...">` 抓取 fallback。
...
DuckDuckGo 的重定向解码在 `decode_duckduckgo_redirect`（[`L2852-L2868`](/rust/crates/tools/src/lib.rs#L2852-L2868)）中处理：支持直接的 `http/https` URL、`//` 协议相对 URL、`/l/` 短链（从 `uddg` 参数提取真实目标）。
...
`dispatch` 方法（[`L235-L296`](/rust/crates/runtime/src/lsp_client.rs#L235-L296)）目前返回结构化的占位结果，真实的 LSP JSON-RPC 调用链路尚未完全接入。这意味着 Rust 实现中 LSP 导航能力的骨架已经搭好（状态追踪、扩展名路由、Action 解析），但完整走到 `rust-analyzer` / `typescript-language-server` 还需要后续工作。
