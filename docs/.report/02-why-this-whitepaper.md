...
参见 [`conversation.rs#L296-L484`](/rust/crates/runtime/src/conversation.rs#L296-L484)：
...
Hooks 系统进一步强化了这个自愈模型。在每次工具调用前后，`run_turn` 都会调用 `run_pre_tool_use_hook` 和 `run_post_tool_use_hook`。如果 hook 返回 `Denied`/`Failed`/`Cancelled`，循环会将该结果作为 `is_error = true` 的 `ToolResult` 回传模型，而不是粗暴终止。参见 [`conversation.rs#L348-L470`](/rust/crates/runtime/src/conversation.rs#L348-L470)。
...
[`session.rs#L28-L52`](/rust/crates/runtime/src/session.rs#L28-L52) 定义了 `ContentBlock` 枚举：
...
`PermissionPolicy::authorize_with_context(...)` 在 `run_turn` 的工具执行路径中被调用（[`conversation.rs#L348-L470`](/rust/crates/runtime/src/conversation.rs#L348-L470)）。如果当前模式是 `Prompt`，系统会调用 `PermissionPrompter` 向用户展示确认对话框。
...
[`bash.rs#L289-L307`](/rust/crates/runtime/src/bash.rs#L289-L307) 定义了 `MAX_OUTPUT_BYTES = 16_384`，`truncate_output()` 会在 Bash 输出超过阈值时截断并追加标记：`[output truncated — exceeded 16384 bytes]`。这是 "单次输出过长" 的即时压缩。
...
压缩后，会话的消息列表会以一条 `MessageRole::System` 的 "continuation message" 开头，其中包含 `Summary`、`Key timeline`、`Tools mentioned`、`Pending work` 等结构化字段（[`compact.rs#L150-L290`](/rust/crates/runtime/src/compact.rs#L150-L290)）。这确保了模型在压缩后仍能理解对话的语义脉络——不是砍掉旧消息，而是用 AI 可读的格式重新编码它们。
