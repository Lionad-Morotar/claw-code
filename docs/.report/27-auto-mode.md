...
参见 [`permissions.rs#L255-L265`](/rust/crates/runtime/src/permissions.rs#L255-L264)。
...
配置解码逻辑在 [`runtime/src/config.rs#L845-L862`](/rust/crates/runtime/src/config.rs#L845-L862)：
...
当模型在一次 turn 中发起 `ToolUse` 时，[`conversation.rs`](/rust/crates/runtime/src/conversation.rs) 的 `run_turn` 会依次经过 hook 处理、权限判定、工具执行三阶段。权限判定的入口在 [`conversation.rs#L400-L410`](/rust/crates/runtime/src/conversation.rs#L400-L410) 附近：
...
完整实现参见 [`permissions.rs#L175-L292`](/rust/crates/runtime/src/permissions.rs#L175-L292)。
...
测试用例 [`permissions.rs#L560-L585`](/rust/crates/runtime/src/permissions.rs#L560-L585) 展示了 `allow/deny` 规则的实际效果：
...
整套管道的入口是 [`validate_command`](/rust/crates/runtime/src/bash_validation.rs#L585-L619)：
...
`permission_enforcer.rs` 在 `PermissionPolicy` 之上增加了一层运行时的执行门控。它在 `Prompt` 模式下有特殊处理：由于 enforcer 本身不带交互式 prompter，它会直接返回 `Allowed`，将确认职责交给上层（即 `conversation.rs` 中的 prompter 分支）。参见 [`permission_enforcer.rs#L41-L52`](/rust/crates/runtime/src/permission_enforcer.rs#L41-L52)：
...
- `check()` —— 在 `Prompt` 模式下直接返回 `Allowed`，将确认职责交给上层的交互式 prompter（[`L41-L52`](/rust/crates/runtime/src/permission_enforcer.rs#L41-L52)）
...
`PermissionContext` 的构造逻辑在 [`conversation.rs#L375-L378`](/rust/crates/runtime/src/conversation.rs#L375-L378) 附近：
...
测试用例 [`permissions.rs#L605-L645`](/rust/crates/runtime/src/permissions.rs#L605-L644) 验证了 hook `Allow` 仍会被 ask 规则覆盖：
