...
`claw-code` 的权限核心定义在 [`runtime/src/permissions.rs#L9-L18`](/rust/crates/runtime/src/permissions.rs#L9-L18)：
...
参见 [`permissions.rs#L255-L265`](/rust/crates/runtime/src/permissions.rs#L255-L265)。
...
整套管道的入口是 [`validate_command`](/rust/crates/runtime/src/bash_validation.rs#L585-L620)：
...
`permission_enforcer.rs` 在 `PermissionPolicy` 之上增加了一层运行时的执行门控。它在 `Prompt` 模式下有特殊处理：由于 enforcer 本身不带交互式 prompter，它会直接返回 `Allowed`，将确认职责交给上层（即 `conversation.rs` 中的 prompter 分支）。参见 [`permission_enforcer.rs#L35-L45`](/rust/crates/runtime/src/permission_enforcer.rs#L35-L45)：
...
测试用例 [`permissions.rs#L605-L645`](/rust/crates/runtime/src/permissions.rs#L605-L645) 验证了 hook `Allow` 仍会被 ask 规则覆盖：
