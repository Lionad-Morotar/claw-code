...
- [`conversation.rs#L23-L27`](/rust/crates/runtime/src/conversation.rs#L23-L27) 定义了 `ApiRequest`，其 `system_prompt` 字段类型正是 `Vec<String>`。
- [`main.rs#L5795-L5802`](/rust/crates/rusty-claude-cli/src/main.rs#L5795-L5802) 的 `build_system_prompt` 包装了 `load_system_prompt`，供 CLI 启动时调用。
- [`types.rs#L6-L34`](/rust/crates/api/src/types.rs#L6-L34) 的 `MessageRequest` 将 `system_prompt` 拼接为单个 `String` 放入 `system` 字段，最终发向上游 API。
...
- `LiveCli::new()` 在 [`main.rs#L3325-L3345`](/rust/crates/rusty-claude-cli/src/main.rs#L3325-L3345) 调用 `build_system_prompt()` 并将结果存入 `LiveCli` 和 `ConversationRuntime`。
- `ConversationRuntime::run_turn()` 在 [`conversation.rs#L322-L324`](/rust/crates/runtime/src/conversation.rs#L322-L324) 将 `self.system_prompt.clone()` 写入 `ApiRequest`。
- `ProviderClient` 在 [`client.rs#L17-L115`](/rust/crates/api/src/client.rs#L17-L115) 根据模型名选择 `AnthropicClient` 或 `OpenAiCompatClient`，两者都消费 `MessageRequest.system`。
...
- `ApiRequest` 定义在 [`conversation.rs#L23-L27`](/rust/crates/runtime/src/conversation.rs#L23-L27)。
- `MessageRequest` 定义在 [`types.rs#L6-L34`](/rust/crates/api/src/types.rs#L6-L34)，`system` 字段为 `Option<String>`，与 `messages` 完全分离。
...
- `environment_section()` 在 [`prompt.rs#L173-L194`](/rust/crates/runtime/src/prompt.rs#L173-L194) 组装环境信息。
...
这意味着 Rust 实现中没有显式的“危险 uncached section”概念——因为整个 `system_prompt` 是构建完成后以 `Vec<String>` 形式绑定到 `ConversationRuntime` 的。如果在会话期间配置或 MCP 发生变化，需要重启 CLI 才能生效。

### 源码映射

- `ProjectContext::discover_with_git()` 在 [`prompt.rs#L81-L90`](/rust/crates/runtime/src/prompt.rs#L81-L90) 一次性收集指令文件和 git 上下文。
- `ConfigLoader::load()` 在 [`config.rs#L271-L340`](/rust/crates/runtime/src/config.rs#L271-L340) 读取并合并所有配置文件。
- `SystemPromptBuilder` 在 [`prompt.rs#L93-L103`](/rust/crates/runtime/src/prompt.rs#L93-L103) 定义，Builder 方法如 `with_project_context`、`with_runtime_config` 在 L125-L135。
...
- `main.rs` 中 `Prompt` 和 `Repl` 的 match 分发在 [`main.rs#L165-L235`](/rust/crates/rusty-claude-cli/src/main.rs#L165-L235) 附近。
- `build_system_prompt()` 在 [`main.rs#L5795-L5802`](/rust/crates/rusty-claude-cli/src/main.rs#L5795-L5802)。
...
- `ProviderClient::from_model_with_anthropic_auth()` 的完整 match 分支在 [`client.rs#L21-L48`](/rust/crates/api/src/client.rs#L21-L48)。

---

## 缓存策略：分块、标记、命中

`claw-code` 的缓存策略比上游更收敛。Anthropic 原生 API 支持服务器端的 Prompt Cache（通过 `cache_control`），而 `claw-code` 额外实现了一层**本地 completion cache**（`prompt_cache.rs`），用于缓存完全相同请求（system + messages + tools）的模型响应，避免重复支付 API 费用。
...
- `PromptCache` struct 定义在 [`prompt_cache.rs#L108-L132`](/rust/crates/api/src/prompt_cache.rs#L108-L132)。
- `detect_cache_break()` 在 [`prompt_cache.rs#L314-L382`](/rust/crates/api/src/prompt_cache.rs#L314-L382)，是缓存失效分析的核心。
- `TrackedPromptState` 在 [`prompt_cache.rs#L268-L293`](/rust/crates/api/src/prompt_cache.rs#L268-L293) 记录每次请求的指纹状态。
- `RequestFingerprints::from_request()` 在 [`prompt_cache.rs#L303-L312`](/rust/crates/api/src/prompt_cache.rs#L303-L312) 计算请求 fingerprint。
...
`ProjectContext` 在 `prompt.rs#L54-L62` 定义，包含：
...
- `ProjectContext` 定义在 [`prompt.rs#L54-L62`](/rust/crates/runtime/src/prompt.rs#L54-L62)。
- `ProjectContext::discover_with_git()` 在 [`prompt.rs#L81-L90`](/rust/crates/runtime/src/prompt.rs#L81-L90) 完成收集。
- `render_project_context()` 在 [`prompt.rs#L288-L328`](/rust/crates/runtime/src/prompt.rs#L288-L328) 将其格式化为 `# Project context` section。
- `GitContext` 定义在 [`git_context.rs#L12-L18`](/rust/crates/runtime/src/git_context.rs#L12-L18)。
...
- `discover_instruction_files()` 在 [`prompt.rs#L203-L224`](/rust/crates/runtime/src/prompt.rs#L203-L224) 实现向上遍历与候选文件收集。
...
- `print_system_prompt()` 输出既支持纯文本拼接，也支持 JSON section 数组：[`main.rs#L2048-L2066`](/rust/crates/rusty-claude-cli/src/main.rs#L2048-L2066)。
...
- `ProviderClient` 分发逻辑在 [`client.rs#L21-L48`](/rust/crates/api/src/client.rs#L21-L48)。
