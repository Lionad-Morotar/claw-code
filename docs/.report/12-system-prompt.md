...
- [`main.rs#L5821-L5827`](/rust/crates/rusty-claude-cli/src/main.rs#L5821-L5827) 的 `build_system_prompt` 包装了 `load_system_prompt`，供 CLI 启动时调用。
...
- `LiveCli::new()` 在 [`main.rs#L3348-L3380`](/rust/crates/rusty-claude-cli/src/main.rs#L3348-L3380) 调用 `build_system_prompt()` 并将结果存入 `LiveCli` 和 `ConversationRuntime`。
...
- `ProviderClient` 在 [`client.rs#L10-L95`](/rust/crates/api/src/client.rs#L10-L95) 根据模型名选择 `AnthropicClient` 或 `OpenAiCompatClient`，两者都消费 `MessageRequest.system`。
...
- `SystemPromptBuilder::build()` 在 [`prompt.rs#L144-L164`](/rust/crates/runtime/src/prompt.rs#L144-L164) 按上述顺序拼接所有 section。
...
- `environment_section()` 在 [`prompt.rs#L173-L193`](/rust/crates/runtime/src/prompt.rs#L173-L193) 组装环境信息。
...
- `main.rs` 中 `Prompt` 和 `Repl` 的 match 分发在 [`main.rs#L168-L240`](/rust/crates/rusty-claude-cli/src/main.rs#L168-L240) 附近。
- `build_system_prompt()` 在 [`main.rs#L5821-L5827`](/rust/crates/rusty-claude-cli/src/main.rs#L5821-L5827)。
...
- `detect_cache_break()` 在 [`prompt_cache.rs#L313-L383`](/rust/crates/api/src/prompt_cache.rs#L313-L383)，是缓存失效分析的核心。
...
- `dedupe_instruction_files()` 在 [`prompt.rs#L353-L364`](/rust/crates/runtime/src/prompt.rs#L353-L364) 基于 `stable_content_hash` 去重。
