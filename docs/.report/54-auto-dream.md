...
> **源码映射说明**：Auto Dream 全部基于 `packages/ccb`（Claude Code 上游 TypeScript 实现）。`claw-code`（Rust 重写版）目前尚未实现此功能。本报告引用的 `packages/ccb/src/...` 路径在上游实现中存在，但在当前仓库中**不存在对应源码文件**。阅读时请注意区分上游与 Rust 实现的覆盖范围。---

## 1. 概述

Auto Dream 是 Claude Code 的后台记忆整合机制。它在会话间自动审查、组织和修剪持久化记忆文件，意图让后续会话更快地加载相关上下文。实际效果取决于 forked agent 的整理质量以及记忆文件本身的 signal-to-noise 比。
...
| 模块 | 路径 | 职责 | 行号 |
|------|------|------|------|
| 调度器 | `src/services/autoDream/autoDream.ts` | 时间/会话/锁三重门控，触发 forked agent | L1-L326 |
| 配置 | `src/services/autoDream/config.ts` | 读取 `isAutoDreamEnabled()` 开关 | L1-L21 |
| 提示词 | `src/services/autoDream/consolidationPrompt.ts` | 构建 4 阶段整理提示词 | L1-L65 |
| 锁文件 | `src/services/autoDream/consolidationLock.ts` | PID 锁 + mtime 作为 `lastConsolidatedAt` | L1-L140 |
| 任务 UI | `src/tasks/DreamTask/DreamTask.ts` | 后台任务注册，footer pill + Shift+Down 可见 | L1-L157 |
| 手动入口 | `src/skills/bundled/dream.ts` | `/dream` 命令，无条件可用 | L1-L44 |
| 路径解析 | `src/memdir/paths.ts` | 记忆目录路径解析 | L1-L278 |
| 进度对话框 | `src/components/tasks/DreamDetailDialog.tsx` | UI 详情展示 | L1-L133 |
...
3. 默认：`<memoryBase>/projects/<sanitized-git-root>/memory/` — L223-L239
...
**约束**:
- Bash 工具限制为只读命令（`ls`、`grep`、`cat` 等）— `L221`
...
新会话启动时，`MEMORY.md` 被加载到上下文中。经过 Dream 整理的记忆文件结构相对清晰，可让 Claude 更快了解用户和项目。
...
| 功能 | 文件 | 行号 |
|------|------|------|
| 入口函数 `executeAutoDream()` | `src/services/autoDream/autoDream.ts` | L321 |
| 初始化 `initAutoDream()` | `src/services/autoDream/autoDream.ts` | L123 |
...
| 会话扫描 `listSessionsTouchedSince()` | `src/services/autoDream/consolidationLock.ts` | L118-L128 |
...
| 记忆路径 `getAutoMemPath()` | `src/memdir/paths.ts` | L223-L239 |
...
| Forked Agent `runForkedAgent()` | `src/utils/forkedAgent.ts` | L489 |
...
*报告生成完毕 — 所有源码锚点基于 `packages/ccb/` 子项目*
