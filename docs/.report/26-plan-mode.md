...
当用户说"重构这个模块"时，传统模式下 AI 会立刻开始修改代码——但此时用户还不清楚 AI 打算怎么改。等改了一半发现方向不对时，已经造成了破坏性变更。

Plan Mode 通过引入一个**只读探索阶段**来解决这个问题：AI 必须先探索代码库、形成书面方案、获得用户批准，然后才能执行修改。
...
| 步骤 | 工具 | 说明 |
| --- | --- | --- |
| 1 | `EnterPlanMode` | AI 自主判断或用户触发任务需要规划，调用此工具进入计划模式（需要用户审批） |
| 2 | 探索阶段 | 权限模式切换为 `'plan'`，AI 只能使用 `isReadOnly()` 为 true 的工具 |
| 3 | `ExitPlanMode` | AI 完成探索后提交计划文件给用户审阅（第二个需要用户审批的节点） |
| 4 | 恢复执行 | 用户批准后，权限模式恢复到进入前的状态，AI 按计划执行 |
...
状态文件路径函数 [`lib.rs#L5075-L5081`](/rust/crates/tools/src/lib.rs#L5075-L5081)：
...
状态数据结构 [`lib.rs#L2491-L2497`](/rust/crates/tools/src/lib.rs#L2491-L2497)：
...
状态读写函数 [`lib.rs#L5083-L5115`](/rust/crates/tools/src/lib.rs#L5083-L5115)：
...
在 `claw-code` 的配置系统中，`"plan"` 模式被解析为 `ReadOnly` 权限级别。参见 [`runtime/src/config.rs#L851-L863`](/rust/crates/runtime/src/config.rs#L851-L863)：
...
[`permission_enforcer.rs#L78-L104`](/rust/crates/runtime/src/permission_enforcer.rs#L78-L104)：
...
[`permission_enforcer.rs#L115-L137`](/rust/crates/runtime/src/permission_enforcer.rs#L115-L137)：
...
在 [`lib.rs#L7958-L8068`](/rust/crates/tools/src/lib.rs#L7958-L8068) 中有两个测试用例验证 EnterPlanMode/ExitPlanMode 的往返行为：
...
| [`/rust/crates/tools/src/lib.rs`](/rust/crates/tools/src/lib.rs) | `PlanModeState` / `PlanModeOutput` 结构 | L2491-L2510 |
...
| [`/rust/crates/tools/src/lib.rs`](/rust/crates/tools/src/lib.rs) | `plan_mode_state_file` 路径函数 | L5075-L5081 |
| [`/rust/crates/tools/src/lib.rs`](/rust/crates/tools/src/lib.rs) | 状态读写清除函数 | L5083-L5115 |
| [`/rust/crates/tools/src/lib.rs`](/rust/crates/tools/src/lib.rs) | 测试用例 | L7958-L8068 |
| [`/rust/crates/runtime/src/config.rs`](/rust/crates/runtime/src/config.rs) | `"plan"` 模式解析为 `ReadOnly` | L851-L863 |
| [`/rust/crates/runtime/src/permission_enforcer.rs`](/rust/crates/runtime/src/permission_enforcer.rs) | 文件写入检查 | L78-L104 |
| [`/rust/crates/runtime/src/permission_enforcer.rs`](/rust/crates/runtime/src/permission_enforcer.rs) | Bash 命令检查 | L115-L137 |
...
1. **状态持久化**：计划模式通过 `plan-mode.json` 文件保存进入前的状态，确保退出时能精确恢复，而不是简单重置为默认值。

2. **工作树局部性**：所有设置都保存在工作树级别的 `settings.local.json` 中，这意味着不同工作树可以有不同的计划模式状态。

3. **非侵入式**：当检测到计划模式已在外部被更改时（`current_is_plan` 但状态文件不存在），工具会清除陈旧状态并优雅退出，而不是强制覆盖用户的选择。

4. **双重审批**：进入计划模式需要审批（因为会改变权限行为），退出时提交计划文件同样需要审批（因为会恢复完整权限并开始实际修改）。
